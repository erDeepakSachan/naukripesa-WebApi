...
{
    public class CurrencyController : NeoController
    {
        private readonly CurrencyService service;

        public CurrencyController(CurrencyService service, WebHelper webHelper, WebHelper.SessionHelper sessionHelper) :
        ...
        public async Task<IActionResult> Add([FromBody]Currency obj)
        {
            ...
            if (success)
            {
                resp = resp.SuccessResponse(null, "Currency has been created successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("Currency create failed.");
            }
        ...
        public async Task<IActionResult> Edit(Currency obj)
        {
            ...
            if (success)
            {
                resp = resp.SuccessResponse(null, "Currency has been updated successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("Currency update failed.");
            }
        ...
        public async Task<IActionResult> Delete(Int32 id)
        {
            ...
            if (sucess)
            {
                resp = resp.SuccessResponse(null, "Currency deleted Successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("Currency delete Failed.");
            }
