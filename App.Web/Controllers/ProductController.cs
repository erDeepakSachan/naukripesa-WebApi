...
{
    public class ProductController : NeoController
    {
        private readonly ProductService service;

        public ProductController(ProductService service, WebHelper webHelper, WebHelper.SessionHelper sessionHelper) :
        ...
        public async Task<IActionResult> Search([FromQuery]string q)
        {
            ...
            var data = await service.GetAll().Where(p => p.Pname.Contains(q)).ToListAsync();
        ...
        public async Task<IActionResult> Add([FromBody]Product obj)
        {
            ...
            if (success)
            {
                resp = resp.SuccessResponse(null, "Product has been created successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("Product create failed.");
            }
        ...
        public async Task<IActionResult> Edit(Product obj)
        {
            ...
            if (success)
            {
                resp = resp.SuccessResponse(null, "Product has been updated successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("Product update failed.");
            }
        ...
        public async Task<IActionResult> Delete(Int32 id)
        {
            ...
            if (sucess)
            {
                resp = resp.SuccessResponse(null, "Product deleted Successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("Product delete Failed.");
            }
