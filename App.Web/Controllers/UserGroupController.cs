...
{
    public class UserGroupController : NeoController
    {
        private readonly UserGroupService service;

        public UserGroupController(UserGroupService service, WebHelper webHelper, WebHelper.SessionHelper sessionHelper) :
        ...
        [Route("")]
        [HttpPost]
        public async Task<IActionResult> Add([FromBody]UserGroup obj)
        {
            var success = await service.Insert(obj);
            var resp = new NeoApiResponse();
            if (success)
            {
                resp = resp.SuccessResponse(null, "User group has been created successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("User group create failed.");
            }
        ...
        [HttpPut]
        [Route("")]
        public async Task<IActionResult> Edit(UserGroup obj)
        {
            var success = await service.Update(obj);
            var resp = new NeoApiResponse();
            if (success)
            {
                resp = resp.SuccessResponse(null, "User group has been updated successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("User group update failed.");
            }
        ...
        public async Task<IActionResult> Delete(Int32 id)
        {
            ...
            if (sucess)
            {
                resp = resp.SuccessResponse(null, "User group deleted Successfully.");
            }
            else
            {
                resp = resp.ErrorResponse("User group delete Failed.");
            }
