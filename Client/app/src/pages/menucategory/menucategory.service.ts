...
export class MenuCategoryService extends PageCommonService {
  private baseUrl: string;
  private emptyListingResponse: ListingResponse<MenuCategory> = { pageSize: 0, totalPageCount: 0, totalItemCount: 0, currentPageNo: 0, data: [] };

  constructor(override http: HttpClient) {
    super(http);
    this.baseUrl = `${this.apiBaseUrl}/menucategory`
  }

  list(pageNo: number = 0): Observable<ListingResponse<MenuCategory>> {
    this.showLoader();
    return this.http.get<{ isSuccess: boolean, data: ListingResponse<MenuCategory> }>(`${this.baseUrl}?pageNo=${pageNo}`, { context: withNoGlobalLoaderGif() }).pipe(
      map((response) => {
  ...
  search(q: string): Observable<ListingResponse<MenuCategory>> {
    this.showLoader();
    return this.http.get<{ isSuccess: boolean, data: ListingResponse<MenuCategory> }>(`${this.baseUrl}/search?q=${q}`, { context: withNoGlobalLoaderGif() }).pipe(
      map((response) => {
        if (response.isSuccess) {
          return response.data;
        } else {
  ...
  add(obj: MenuCategory): Observable<{ isSuccess: boolean, message: string }> {
    this.showLoader();
  ...
  edit(obj: MenuCategory): Observable<{ isSuccess: boolean, message: string }> {
    this.showLoader();
